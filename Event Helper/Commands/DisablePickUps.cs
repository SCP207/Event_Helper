...
public class DisablePickUps : ICommand, IUsageProvider {
    public string Command => "disablepickups";
    public string[] Aliases => [ "dpu", "dp" ];
    public string Description => "Like bypass, but allows a player to lock a door";
    public string[] Usage => [ "Add / Remove", "Item IDs", "%player%" ];
    ...
    public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response) {
        ...
        foreach (ItemType i in items) {
            if (arguments.At(0) == "add") {
                ...
                foreach (var p in players) {
                    for (int index = 0; index < 8; index++) {
                        var item = p.Items.FirstOrDefault(item => item.Type == i);
                        if (item == null) break;
            ...
            playerList = [.. playerList.Distinct()];
